...
### ステップ3: HTMLファイルの生成と公開
総括を追記した後、以下のコマンドを実行してHTMLファイルを生成します：

```bash
# 議事録HTMLファイルの生成
./create-minute.sh

# または、特定の日付のログファイルを指定する場合
./create-minute.sh logs/20250819_send.log
```

このスクリプトは以下の処理を実行します：
1. `logs/`ディレクトリから議事録を読み込み
2. Markdown部分をHTMLに変換
3. `minutes/年/月/`ディレクトリ構造でHTMLファイルを保存
4. `minutes/index.html`に議事録一覧を自動更新
